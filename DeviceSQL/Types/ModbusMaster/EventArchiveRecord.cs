...
using System.Text;
    ...
    public partial class UserDefinedTypes
    {
        ...
        public struct EventArchiveRecord : INullable, IBinarySerialize
        {
            ...
            {
                return new Device.Modbus.Data.EventArchiveRecord(Convert.ToUInt16(Index), Data.Value).GetShortValue(index.Value, byteSwap.Value);
            }

            public static SqlDateTime GetDateTimeValue(SqlSingle dateValue, SqlSingle timeValue, SqlInt32 baseYear)
            {
                var dateTime = Device.Modbus.Data.EventArchiveRecord.ParseNullableDateTimeValue(dateValue.Value, timeValue.Value, baseYear.Value);
                return dateTime ?? SqlDateTime.Null;
            }

            public SqlSingle GetFloatValue(SqlByte index, SqlBoolean byteSwap, SqlBoolean wordSwap)
            {
                var floatValue = new Device.Modbus.Data.EventArchiveRecord(Convert.ToUInt16(Index), Data.Value).GetNullableFloatValue(index.Value, byteSwap.Value, wordSwap.Value);
