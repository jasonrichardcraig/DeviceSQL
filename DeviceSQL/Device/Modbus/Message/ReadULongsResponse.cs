...
{
    internal class ReadULongsResponse : ModbusMessage, IModbusResponseMessage
    {
        ...
        void IModbusResponseMessage.Initialize(byte[] frame, bool isExtendedUnitId, IModbusRequestMessage requestMessage)
        {
            ...
            var readULongsRequest = requestMessage as ReadULongsRequest;

            if (readULongsRequest != null)
            {
                this.ULongRegisters = readULongsRequest.ULongRegisters;
                ...
                var uLongRegisters = new List<LongRegister>();

                for (int i = 1; i < data.Length; i += 4)
                {
                    var uLongData = new byte[] { data[i + 3], data[i + 2], data[i + 1], data[i], };
                    ((IModbusRegisterData)this.ULongRegisters[((i - 1) / 4)]).Data = uLongData;
                }
